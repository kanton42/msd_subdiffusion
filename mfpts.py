# from Lucas T. Tepper used for Ayaz et al PNAS 2021 (https://github.com/lucastepper/memtools)
...
def is_transition(x1, x2, barrier):
    """ Determine if transition through barrier occurs form x1 to x2. """
...
def get_ffpt_single(traj, start, end):
    """ Obtain first-first passage time, as used for Klimek et al. PNAS 2025. """
    count = 0
    ffpt_sum = 0.0
    start_found = -1
...
def get_mfpt(trajs, edges_start, edges_end, dt, return_sum=False):
    """ Obtain mean-first passage time from trajs. """
...
def get_mffpt(trajs, edges_start, edges_end, dt, return_sum=False):
    """ Obtain MFPt (first-fisrt) as used in Klimek et al. PNAS 2025. """
    if isinstance(trajs, np.ndarray) and trajs.ndim > 1:
        pass
    elif not isinstance(trajs, (list, tuple)):
        trajs = [trajs]
